...
import org.ovirt.engine.sdk4.services.AffinityGroupHostLabelService;
...
public class AffinityGroupHostLabelsServiceImpl extends ServiceImpl implements AffinityGroupHostLabelsService {
    ...
    private class AddRequestImpl extends BaseRequest<AddRequest, AddResponse> implements AddRequest {
        ...
        public AddRequest label(AffinityLabel label) {
            this.label = label;
            return this;
        }
        
        public AddRequest label(AffinityLabelBuilder label) {
        ...
        public AddResponse send() {
            ...
            try (
              ...
              XmlWriter xmlWriter = new XmlWriter(output, true)
                ...
                XmlAffinityLabelWriter.writeOne(label, xmlWriter);
                xmlWriter.flush();
              ...
              response.getStatusLine().getStatusCode() == 200
                ...
                try (
                  XmlReader reader = new XmlReader(response.getEntity().getContent())
                    ...
                    actionResponse.labels(XmlAffinityLabelReader.readMany(reader));
                    return actionResponse;
                }
                catch (IOException ex) {
                    throw new Error("Failed to read response", ex);
    ...
    private class ListResponseImpl implements ListResponse {
        private List<AffinityLabel> labels;
        
        public void labels(List<AffinityLabel> labels) {
            this.labels = labels;
        }
        
        public List<AffinityLabel> labels() {
            return labels;
        }
    ...
    public AffinityGroupHostLabelService labelService(String id) {
        return new AffinityGroupHostLabelServiceImpl(getConnection(), getPath() + "/" + id);
    }
    ...
    public Service service(String path) {
        ...
        if (index == -1) {
            return labelService(path);
        }
        return labelService(path.substring(0, index)).service(path.substring(index + 1));
    }
    
    public String toString() {
        return "AffinityGroupHostLabelsService:" + getPath();
    }
