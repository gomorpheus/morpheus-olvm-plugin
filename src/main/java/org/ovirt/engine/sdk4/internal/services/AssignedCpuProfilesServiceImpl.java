...
import org.ovirt.engine.sdk4.services.AssignedCpuProfileService;
import org.ovirt.engine.sdk4.services.AssignedCpuProfilesService;
import org.ovirt.engine.sdk4.types.CpuProfile;

public class AssignedCpuProfilesServiceImpl extends ServiceImpl implements AssignedCpuProfilesService {
    
    public AssignedCpuProfilesServiceImpl(HttpConnection connection, String path) {
    ...
    private class AddRequestImpl extends BaseRequest<AddRequest, AddResponse> implements AddRequest {
        ...
        public AddRequest profile(CpuProfile profile) {
            this.profile = profile;
            return this;
        }
        
        public AddRequest profile(CpuProfileBuilder profile) {
              ...
              response.getStatusLine().getStatusCode() == 200
                ...
                try (
                  XmlReader reader = new XmlReader(response.getEntity().getContent())
                    ...
                    actionResponse.profiles(XmlCpuProfileReader.readMany(reader));
                    return actionResponse;
                }
                catch (IOException ex) {
                    throw new Error("Failed to read response", ex);
    ...
    private class ListResponseImpl implements ListResponse {
        private List<CpuProfile> profiles;
        
        public void profiles(List<CpuProfile> profiles) {
            this.profiles = profiles;
        }
        
        public List<CpuProfile> profiles() {
            return profiles;
        }
    ...
    public AssignedCpuProfileService profileService(String id) {
        return new AssignedCpuProfileServiceImpl(getConnection(), getPath() + "/" + id);
    }
    ...
    public Service service(String path) {
        ...
        if (index == -1) {
            return profileService(path);
        }
        return profileService(path.substring(0, index)).service(path.substring(index + 1));
    }
    
    public String toString() {
        return "AssignedCpuProfilesService:" + getPath();
    }
