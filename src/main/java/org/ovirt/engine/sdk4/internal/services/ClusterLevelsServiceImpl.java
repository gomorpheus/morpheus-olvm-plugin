...
public class ClusterLevelsServiceImpl extends ServiceImpl implements ClusterLevelsService {
    ...
    public ClusterLevelService levelService(String id) {
        return new ClusterLevelServiceImpl(getConnection(), getPath() + "/" + id);
    }
    ...
    public Service service(String path) {
        ...
        if (index == -1) {
            return levelService(path);
        }
        return levelService(path.substring(0, index)).service(path.substring(index + 1));
    }
    
    public String toString() {
        return "ClusterLevelsService:" + getPath();
    }
