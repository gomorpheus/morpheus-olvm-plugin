...
public class CpuProfilesServiceImpl extends ServiceImpl implements CpuProfilesService {
              ...
              response.getStatusLine().getStatusCode() == 200
                ...
                try (
                  XmlReader reader = new XmlReader(response.getEntity().getContent())
                    ...
                    actionResponse.profile(XmlCpuProfileReader.readMany(reader));
                    return actionResponse;
                }
                catch (IOException ex) {
                    throw new Error("Failed to read response", ex);
    ...
    private class ListResponseImpl implements ListResponse {
        private List<CpuProfile> profile;
        
        public void profile(List<CpuProfile> profile) {
            this.profile = profile;
        }
        
        public List<CpuProfile> profile() {
            return profile;
        }
    ...
    public CpuProfileService profileService(String id) {
        return new CpuProfileServiceImpl(getConnection(), getPath() + "/" + id);
    }
    ...
    public String toString() {
        return "CpuProfilesService:" + getPath();
    }
