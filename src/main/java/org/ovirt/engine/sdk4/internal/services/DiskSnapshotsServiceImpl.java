...
import org.ovirt.engine.sdk4.services.DiskSnapshotService;
...
public class DiskSnapshotsServiceImpl extends ServiceImpl implements DiskSnapshotsService {
    ...
    private class ListRequestImpl extends BaseRequest<ListRequest, ListResponse> implements ListRequest {
        ...
        public ListRequest includeActive(Boolean includeActive) {
            this.includeActive = includeActive;
            return this;
        }
        
        private Boolean includeTemplate;
        public ListRequest includeTemplate(Boolean includeTemplate) {
        ...
        public ListResponse send() {
            ...
            try {
                ...
                if (includeActive != null) {
                    uriBuilder.addParameter("include_active", XmlWriter.renderBoolean(includeActive));
                }
                if (includeTemplate != null) {
                    uriBuilder.addParameter("include_template", XmlWriter.renderBoolean(includeTemplate));
                }
                if (max != null) {
              ...
              response.getStatusLine().getStatusCode() == 200
                ...
                try (
                  XmlReader reader = new XmlReader(response.getEntity().getContent())
                    ...
                    actionResponse.snapshots(XmlDiskSnapshotReader.readMany(reader));
    ...
    private class ListResponseImpl implements ListResponse {
        private List<DiskSnapshot> snapshots;
        
        public void snapshots(List<DiskSnapshot> snapshots) {
            this.snapshots = snapshots;
        }
        
        public List<DiskSnapshot> snapshots() {
            return snapshots;
        }
    ...
    public DiskSnapshotService snapshotService(String id) {
        return new DiskSnapshotServiceImpl(getConnection(), getPath() + "/" + id);
    }
    ...
    public Service service(String path) {
        ...
        return snapshotService(path.substring(0, index)).service(path.substring(index + 1));
    }
    
    public String toString() {
        return "DiskSnapshotsService:" + getPath();
    }
