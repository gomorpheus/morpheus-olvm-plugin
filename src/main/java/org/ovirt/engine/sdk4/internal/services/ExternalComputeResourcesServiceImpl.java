...
public class ExternalComputeResourcesServiceImpl extends ServiceImpl implements ExternalComputeResourcesService {
    ...
    public ExternalComputeResourceService resourceService(String id) {
        return new ExternalComputeResourceServiceImpl(getConnection(), getPath() + "/" + id);
    }
    ...
    public Service service(String path) {
        ...
        if (index == -1) {
            return resourceService(path);
        }
        return resourceService(path.substring(0, index)).service(path.substring(index + 1));
    }
    
    public String toString() {
        return "ExternalComputeResourcesService:" + getPath();
    }
