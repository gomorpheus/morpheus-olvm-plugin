...
public class ExternalHostProvidersServiceImpl extends ServiceImpl implements ExternalHostProvidersService {
    
    public ExternalHostProvidersServiceImpl(HttpConnection connection, String path) {
    ...
    private class AddRequestImpl extends BaseRequest<AddRequest, AddResponse> implements AddRequest {
        ...
        public AddRequest provider(ExternalHostProvider provider) {
            this.provider = provider;
            return this;
        }
        
        public AddRequest provider(ExternalHostProviderBuilder provider) {
              ...
              response.getStatusLine().getStatusCode() == 200
                ...
                try (
                  XmlReader reader = new XmlReader(response.getEntity().getContent())
                    ...
                    actionResponse.providers(XmlExternalHostProviderReader.readMany(reader));
                    return actionResponse;
                }
                catch (IOException ex) {
                    throw new Error("Failed to read response", ex);
    ...
    private class ListResponseImpl implements ListResponse {
        private List<ExternalHostProvider> providers;
        
        public void providers(List<ExternalHostProvider> providers) {
            this.providers = providers;
        }
        
        public List<ExternalHostProvider> providers() {
            return providers;
        }
    ...
    public ExternalHostProviderService providerService(String id) {
        return new ExternalHostProviderServiceImpl(getConnection(), getPath() + "/" + id);
    }
    ...
    public String toString() {
        return "ExternalHostProvidersService:" + getPath();
    }
