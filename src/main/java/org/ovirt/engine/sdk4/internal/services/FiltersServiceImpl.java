...
import org.ovirt.engine.sdk4.internal.xml.XmlFilterReader;
import org.ovirt.engine.sdk4.internal.xml.XmlFilterWriter;
import org.ovirt.engine.sdk4.services.FilterService;
import org.ovirt.engine.sdk4.services.FiltersService;
import org.ovirt.engine.sdk4.types.Filter;

public class FiltersServiceImpl extends ServiceImpl implements FiltersService {
    
    public FiltersServiceImpl(HttpConnection connection, String path) {
        super(connection, path);
    }
    
    private class AddRequestImpl extends BaseRequest<AddRequest, AddResponse> implements AddRequest {
        private Filter filter;
        public AddRequest filter(Filter filter) {
            this.filter = filter;
            return this;
        }
        
        public AddRequest filter(FilterBuilder filter) {
        ...
        public AddResponse send() {
            ...
            try (
              ...
              XmlWriter xmlWriter = new XmlWriter(output, true)
                ...
                XmlFilterWriter.writeOne(filter, xmlWriter);
                xmlWriter.flush();
              ...
              || response.getStatusLine().getStatusCode() == 202
                ...
                try (
                  XmlReader reader = new XmlReader(response.getEntity().getContent())
                    ...
                    actionResponse.filter(XmlFilterReader.readOne(reader));
              ...
              response.getStatusLine().getStatusCode() == 200
                ...
                try (
                  XmlReader reader = new XmlReader(response.getEntity().getContent())
                    ...
                    actionResponse.filters(XmlFilterReader.readMany(reader));
                    return actionResponse;
                }
                catch (IOException ex) {
                    throw new Error("Failed to read response", ex);
    ...
    public FilterService filterService(String id) {
        return new FilterServiceImpl(getConnection(), getPath() + "/" + id);
    }
    ...
    public Service service(String path) {
        ...
        return filterService(path.substring(0, index)).service(path.substring(index + 1));
    }
    
    public String toString() {
        return "FiltersService:" + getPath();
    }
