...
public class InstanceTypeNicsServiceImpl extends ServiceImpl implements InstanceTypeNicsService {
    
    public InstanceTypeNicsServiceImpl(HttpConnection connection, String path) {
        super(connection, path);
    }
    
    private class AddRequestImpl extends BaseRequest<AddRequest, AddResponse> implements AddRequest {
        private Nic nic;
        public AddRequest nic(Nic nic) {
            this.nic = nic;
            return this;
        }
        
        public AddRequest nic(NicBuilder nic) {
            this.nic = nic.build();
            return this;
        }
    ...
    public InstanceTypeNicService nicService(String id) {
        return new InstanceTypeNicServiceImpl(getConnection(), getPath() + "/" + id);
    }
    ...
    public Service service(String path) {
        ...
        if (index == -1) {
            return nicService(path);
        }
        return nicService(path.substring(0, index)).service(path.substring(index + 1));
    }
    
    public String toString() {
        return "InstanceTypeNicsService:" + getPath();
    }
