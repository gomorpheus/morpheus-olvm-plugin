...
import org.ovirt.engine.sdk4.services.InstanceTypeWatchdogService;
import org.ovirt.engine.sdk4.services.InstanceTypeWatchdogsService;
import org.ovirt.engine.sdk4.types.Watchdog;

public class InstanceTypeWatchdogsServiceImpl extends ServiceImpl implements InstanceTypeWatchdogsService {
    
    public InstanceTypeWatchdogsServiceImpl(HttpConnection connection, String path) {
        super(connection, path);
    }
    
    private class AddRequestImpl extends BaseRequest<AddRequest, AddResponse> implements AddRequest {
        private Watchdog watchdog;
        public AddRequest watchdog(Watchdog watchdog) {
            this.watchdog = watchdog;
            return this;
        }
        
        public AddRequest watchdog(WatchdogBuilder watchdog) {
        ...
        public AddResponse send() {
            ...
            try (
              ...
              XmlWriter xmlWriter = new XmlWriter(output, true)
                ...
                XmlWatchdogWriter.writeOne(watchdog, xmlWriter);
                xmlWriter.flush();
    ...
    public InstanceTypeWatchdogService watchdogService(String id) {
        return new InstanceTypeWatchdogServiceImpl(getConnection(), getPath() + "/" + id);
    }
    ...
    public Service service(String path) {
        ...
        if (index == -1) {
            return watchdogService(path);
        }
        return watchdogService(path.substring(0, index)).service(path.substring(index + 1));
    }
    
    public String toString() {
        return "InstanceTypeWatchdogsService:" + getPath();
    }
