...
import org.ovirt.engine.sdk4.internal.xml.XmlJobWriter;
...
public class JobsServiceImpl extends ServiceImpl implements JobsService {
    ...
    private class AddRequestImpl extends BaseRequest<AddRequest, AddResponse> implements AddRequest {
        private Job job;
        public AddRequest job(Job job) {
            this.job = job;
            return this;
        }
        
        public AddRequest job(JobBuilder job) {
            this.job = job.build();
            return this;
        }
        ...
        public AddResponse send() {
            ...
            try (
              ...
              XmlWriter xmlWriter = new XmlWriter(output, true)
                ...
                XmlJobWriter.writeOne(job, xmlWriter);
                xmlWriter.flush();
              ...
              response.getStatusLine().getStatusCode() == 200
                ...
                try (
                  XmlReader reader = new XmlReader(response.getEntity().getContent())
                    ...
                    actionResponse.jobs(XmlJobReader.readMany(reader));
                    return actionResponse;
                }
                catch (IOException ex) {
                    throw new Error("Failed to read response", ex);
    ...
    public JobService jobService(String id) {
        return new JobServiceImpl(getConnection(), getPath() + "/" + id);
    }
    ...
    public Service service(String path) {
        ...
        return jobService(path.substring(0, index)).service(path.substring(index + 1));
    }
    
    public String toString() {
        return "JobsService:" + getPath();
    }
