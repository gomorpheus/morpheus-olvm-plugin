...
public class OpenstackNetworkProvidersServiceImpl extends ServiceImpl implements OpenstackNetworkProvidersService {
    
    public OpenstackNetworkProvidersServiceImpl(HttpConnection connection, String path) {
    ...
    private class AddRequestImpl extends BaseRequest<AddRequest, AddResponse> implements AddRequest {
        ...
        public AddRequest provider(OpenStackNetworkProvider provider) {
            this.provider = provider;
            return this;
        }
        
        public AddRequest provider(OpenStackNetworkProviderBuilder provider) {
              ...
              response.getStatusLine().getStatusCode() == 200
                ...
                try (
                  XmlReader reader = new XmlReader(response.getEntity().getContent())
                    ...
                    actionResponse.providers(XmlOpenStackNetworkProviderReader.readMany(reader));
                    return actionResponse;
                }
                catch (IOException ex) {
                    throw new Error("Failed to read response", ex);
    ...
    private class ListResponseImpl implements ListResponse {
        private List<OpenStackNetworkProvider> providers;
        
        public void providers(List<OpenStackNetworkProvider> providers) {
            this.providers = providers;
        }
        
        public List<OpenStackNetworkProvider> providers() {
            return providers;
        }
    ...
    public OpenstackNetworkProviderService providerService(String id) {
        return new OpenstackNetworkProviderServiceImpl(getConnection(), getPath() + "/" + id);
    }
    ...
    public Service service(String path) {
        ...
        if (index == -1) {
            return providerService(path);
        }
        return providerService(path.substring(0, index)).service(path.substring(index + 1));
    }
    
    public String toString() {
        return "OpenstackNetworkProvidersService:" + getPath();
    }
