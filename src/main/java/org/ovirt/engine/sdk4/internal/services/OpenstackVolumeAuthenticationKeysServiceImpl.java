...
public class OpenstackVolumeAuthenticationKeysServiceImpl extends ServiceImpl implements OpenstackVolumeAuthenticationKeysService {
    ...
    private class AddRequestImpl extends BaseRequest<AddRequest, AddResponse> implements AddRequest {
        ...
        public AddRequest key(OpenstackVolumeAuthenticationKey key) {
            this.key = key;
            return this;
        }
        
        public AddRequest key(OpenstackVolumeAuthenticationKeyBuilder key) {
            this.key = key.build();
            return this;
        }
              ...
              response.getStatusLine().getStatusCode() == 200
                ...
                try (
                  XmlReader reader = new XmlReader(response.getEntity().getContent())
                    ...
                    actionResponse.keys(XmlOpenstackVolumeAuthenticationKeyReader.readMany(reader));
                    return actionResponse;
                }
                catch (IOException ex) {
                    throw new Error("Failed to read response", ex);
    ...
    private class ListResponseImpl implements ListResponse {
        private List<OpenstackVolumeAuthenticationKey> keys;
        
        public void keys(List<OpenstackVolumeAuthenticationKey> keys) {
            this.keys = keys;
        }
        
        public List<OpenstackVolumeAuthenticationKey> keys() {
            return keys;
        }
    ...
    public OpenstackVolumeAuthenticationKeyService keyService(String id) {
        return new OpenstackVolumeAuthenticationKeyServiceImpl(getConnection(), getPath() + "/" + id);
    }
    ...
    public Service service(String path) {
        ...
        if (index == -1) {
            return keyService(path);
        }
        return keyService(path.substring(0, index)).service(path.substring(index + 1));
    }
    
    public String toString() {
        return "OpenstackVolumeAuthenticationKeysService:" + getPath();
    }
