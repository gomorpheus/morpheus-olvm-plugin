...
public class QossServiceImpl extends ServiceImpl implements QossService {
    
    public QossServiceImpl(HttpConnection connection, String path) {
        super(connection, path);
    }
    
    private class AddRequestImpl extends BaseRequest<AddRequest, AddResponse> implements AddRequest {
        private Qos qos;
        public AddRequest qos(Qos qos) {
            this.qos = qos;
            return this;
        }
        
        public AddRequest qos(QosBuilder qos) {
            this.qos = qos.build();
            return this;
        }
              ...
              response.getStatusLine().getStatusCode() == 200
                ...
                try (
                  XmlReader reader = new XmlReader(response.getEntity().getContent())
                    ...
                    actionResponse.qoss(XmlQosReader.readMany(reader));
                    return actionResponse;
                }
                catch (IOException ex) {
                    throw new Error("Failed to read response", ex);
    ...
    private class ListResponseImpl implements ListResponse {
        ...
        public void qoss(List<Qos> qoss) {
            this.qoss = qoss;
        }
    ...
    public QosService qosService(String id) {
        return new QosServiceImpl(getConnection(), getPath() + "/" + id);
    }
    ...
    public Service service(String path) {
        ...
        return qosService(path.substring(0, index)).service(path.substring(index + 1));
    }
    
    public String toString() {
        return "QossService:" + getPath();
    }
