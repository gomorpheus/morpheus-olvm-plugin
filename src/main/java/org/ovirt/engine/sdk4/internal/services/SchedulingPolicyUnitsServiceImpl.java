...
public class SchedulingPolicyUnitsServiceImpl extends ServiceImpl implements SchedulingPolicyUnitsService {
    ...
    public SchedulingPolicyUnitService unitService(String id) {
        return new SchedulingPolicyUnitServiceImpl(getConnection(), getPath() + "/" + id);
    }
    ...
    public Service service(String path) {
        ...
        if (index == -1) {
            return unitService(path);
        }
        return unitService(path.substring(0, index)).service(path.substring(index + 1));
    }
    
    public String toString() {
        return "SchedulingPolicyUnitsService:" + getPath();
    }
