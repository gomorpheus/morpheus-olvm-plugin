...
import org.ovirt.engine.sdk4.internal.xml.XmlStepWriter;
...
public class StepsServiceImpl extends ServiceImpl implements StepsService {
    ...
    private class AddRequestImpl extends BaseRequest<AddRequest, AddResponse> implements AddRequest {
        private Step step;
        public AddRequest step(Step step) {
            this.step = step;
            return this;
        }
        
        public AddRequest step(StepBuilder step) {
        ...
        public AddResponse send() {
            ...
            try (
              ...
              XmlWriter xmlWriter = new XmlWriter(output, true)
                ...
                XmlStepWriter.writeOne(step, xmlWriter);
                xmlWriter.flush();
              ...
              response.getStatusLine().getStatusCode() == 200
                ...
                try (
                  XmlReader reader = new XmlReader(response.getEntity().getContent())
                    ...
                    actionResponse.steps(XmlStepReader.readMany(reader));
                    return actionResponse;
                }
                catch (IOException ex) {
                    throw new Error("Failed to read response", ex);
    ...
    public StepService stepService(String id) {
        return new StepServiceImpl(getConnection(), getPath() + "/" + id);
    }
    ...
    public Service service(String path) {
        ...
        return stepService(path.substring(0, index)).service(path.substring(index + 1));
    }
    
    public String toString() {
        return "StepsService:" + getPath();
    }
