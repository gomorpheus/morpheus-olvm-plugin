...
public class VmApplicationsServiceImpl extends ServiceImpl implements VmApplicationsService {
              ...
              response.getStatusLine().getStatusCode() == 200
                ...
                try (
                  XmlReader reader = new XmlReader(response.getEntity().getContent())
                    ...
                    actionResponse.applications(XmlApplicationReader.readMany(reader));
    ...
    private class ListResponseImpl implements ListResponse {
        ...
        public void applications(List<Application> applications) {
    ...
    public VmApplicationService applicationService(String id) {
        return new VmApplicationServiceImpl(getConnection(), getPath() + "/" + id);
    }
    ...
    public Service service(String path) {
        ...
        if (index == -1) {
            return applicationService(path);
        }
        return applicationService(path.substring(0, index)).service(path.substring(index + 1));
    }
    
    public String toString() {
        return "VmApplicationsService:" + getPath();
    }
