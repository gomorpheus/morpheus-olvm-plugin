...
public class XmlClusterLevelReader {
    ...
    public static ClusterLevel readOne(XmlReader reader) {
        ...
        while (reader.forward()) {
            ...
            switch (name) {
                ...
                object.clusterFeatures(XmlClusterFeatureReader.readMany(reader));
                break;
                case "comment":
    ...
    public static Iterator<ClusterLevel> iterateMany(final XmlReader reader) {
        return new Iterator<ClusterLevel>() {
            ...
            public ClusterLevel next() {
                ClusterLevel next = readOne(reader);
                if (next == null) {
    ...
    public static List<ClusterLevel> readMany(XmlReader reader) {
        List<ClusterLevel> list = new ArrayList<>();
        Iterator<ClusterLevel> iterator = iterateMany(reader);
    ...
    private static void processLink(ClusterLevelContainer object, String[] link) {
        ...
        if (href != null) {
            switch (rel) {
                ...
                list = new ArrayListWithHref(object.clusterFeatures());
                list.href(href);
                object.clusterFeatures(list);
                break;
                default:
                break;
            }
