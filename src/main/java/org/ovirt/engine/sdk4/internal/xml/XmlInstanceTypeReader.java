...
public class XmlInstanceTypeReader {
    
    public static InstanceType readOne(XmlReader reader) {
        ...
        // Create the object:
        InstanceTypeContainer object = new InstanceTypeContainer();
    ...
    public static Iterator<InstanceType> iterateMany(final XmlReader reader) {
        return new Iterator<InstanceType>() {
            ...
            public InstanceType next() {
                InstanceType next = readOne(reader);
                if (next == null) {
    ...
    public static List<InstanceType> readMany(XmlReader reader) {
        List<InstanceType> list = new ArrayList<>();
        Iterator<InstanceType> iterator = iterateMany(reader);
    ...
    private static void processLink(InstanceTypeContainer object, String[] link) {
        // Process the attributes:
        ListWithHref list;
        String rel = link[0];
        String href = link[1];
